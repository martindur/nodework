...
fn view_menu_item(item: #(String, String), spawn_func: fn(Dynamic) -> Result(msg, List(DecodeError))) -> element.Element(msg) {
  let #(text, id) = item
  html.button(
    [
      attribute.id(id),
...
pub fn view_menu(menu: Menu, spawn_func: fn(Dynamic) -> Result(msg, List(DecodeError))) -> element.Element(msg) {
  let pos =
    "translate("
  ...
  html.div(
    case menu.active {
      True -> [
        attribute.class("absolute top-0 left-0 w-[100px] h-[300px] bg-gray-200 rounded shadow"),
